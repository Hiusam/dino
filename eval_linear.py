...
import utils
from utils import data_loaders_names
import vision_transformer as vits


from timm.utils import accuracy as t_accuracy
...
def eval_linear(args):
    ...
    # ============ prepare imagenet-a and imagenet-c ======
    # imagenet-a
    ina_dataset = torchvision.datasets.ImageFolder(args.ina_path, transform=build_test_transform(args))
    ina_data_loader = torch.utils.data.DataLoader(
                            ina_dataset, batch_size=int(1.5 * args.batch_size_per_gpu),
                            num_workers=args.num_workers,
                            pin_memory=True,
                            drop_last=False
                        )
    print(f"ImageNet-A: Data loaded with {len(ina_dataset)} imgs.")
    ...
    for name, path in data_loaders_names.items():
        for severity in range(1, 6):
            ...
            inc_data_loader = torch.utils.data.DataLoader(
                            inc_dataset, batch_size=int(1.5 * args.batch_size_per_gpu),
                            num_workers=args.num_workers,
                            pin_memory=True,
                            drop_last=False
                        )
            inc_data_loaders[os.path.join(args.inc_path, path, str(severity))] = inc_data_loader    
    print(f"ImageNet-C: Data loaded with {len(inc_data_loaders.keys()) / 5} kinds of imgs.")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # test_inc(args, inc_data_loaders, model, linear_classifier)
        ...
        if epoch % args.test_robust_freq == 0 or epoch == args.epochs - 1:
            # for out-of-distribution data
            # need optimize
            # 2. store log
            test_ina(args, ina_data_loader, model, linear_classifier)        
...
@torch.no_grad()
def validate_network(val_loader, model, linear_classifier, n, avgpool, mask=None):
    ...
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        if mask is None:
            ...
            # for imagenet-c
            acc1, acc5 = utils.accuracy(output[:, mask], target, topk=(1, 5))
...
@torch.no_grad()
def test_ina(args, ina_data_loader, model, linear_classifier):
    print("Test ImageNet-A")
    ...
    test_stats = validate_network(ina_data_loader, model, linear_classifier, args.n_last_blocks, args.avgpool_patchtokens, mask=imagenet_a_mask)
    print(f"Accuracy on the ImageNet-A: {test_stats['acc1']:.1f}%")


@torch.no_grad()
def test_inc(args, inc_data_loaders:dict, model, linear_classifier):
    print("Test ImageNet-C")
    ...
    for name, path in data_loaders_names.items():
        for severity in range(1, 6):
            test_stats = validate_network(inc_data_loaders[os.path.join(args.inc_path, path, str(severity))], model, linear_classifier, args.n_last_blocks, args.avgpool_patchtokens)
...
@torch.no_grad()
def evaluate(data_loader, model, linear_classifier, mask=None, adv=None):
    # copy from Robust-Vision-Transformer
    ...
    for images, target in metric_logger.log_every(data_loader, 10, header):
        ...
        if mask is None:
            acc1, acc5 = t_accuracy(output, target, topk=(1, 5))
        else:
            acc1, acc5 = t_accuracy(output[:,mask], target, topk=(1, 5))
...
def build_test_transform(args):
    ...
    if resize_im:
        ...
        t.append(
            pth_transforms.Resize(size, interpolation=3),  # to maintain same ratio w.r.t. 224 images
        )
        t.append(pth_transforms.CenterCrop(args.input_size))

    t.append(pth_transforms.ToTensor())
    t.append(pth_transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ins_path', default=None, type=str, help='imagenet-stylized')
    parser.add_argument('--num_workers', default=10, type=int, help='Number of data loading workers per GPU.')
    parser.add_argument('--test_robust_freq', default=5, type=int, help="Epoch frequency for robustness test.")
